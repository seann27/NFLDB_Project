...
class NFLAPI_Processor:
	...
	def process_nflapi(self):
		...
		self.mpd['game_date'] = get_date(self.nflapi)
		...
		for drive in drives:
			...
			for play in plays:
				...
				yrdline = get_yardline_100(details['yrdln'],details['posteam'])
				goal_to_go = 0
				if yrdline == details['ydstogo']:
					goal_to_go = 1
				self.mpd['yardline_100'].append(yrdline)
				self.mpd['goal_to_go'].append(goal_to_go)
				...
				self.mpd['penalty'].append(p['penalty'])
				...
				self.mpd['punt_blocked'].append(st['punt_blocked'])
