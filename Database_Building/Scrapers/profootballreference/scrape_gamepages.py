		...
		for game in games:
    		...
    		for key,val in player_list.items():
    			if hasattr(val,'snapcount'):
    				...
    				sql += "id,game_id,player_id,player_teamid,opponent,"
    				...
    				sql += "pass_att=(%s),pass_comp=(%s),pass_yards=(%s),pass_tds=(%s),pass_int=(%s),"
    				sql += "rush_att=(%s),rush_yards=(%s),rush_tds=(%s),"
    				sql += "rec_tgts=(%s),rec=(%s),rec_yards=(%s),rec_tds=(%s),"
    				sql += "rtn_tds=(%s),extra_points=(%s),fum_lost=(%s),snapcount=(%s),pct_snaps=(%s),dk_points=(%s)"
    				sql_val = (val.id,val.game_id,val.player_id,val.player_teamid,val.opponent,
