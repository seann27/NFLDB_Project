...
def get_players(pos):
	...
	for item in results:
		...
		if pos == 'D':
			obj.dst = 1
		player_stats_list.append((name,rank,salary,obj.qb,obj.rb,obj.wr,obj.te,obj.dst))

get_players('QB')
# player_stats_list = player_stats_list[:12]
get_players('RB')
get_players('WR')
get_players('TE')
get_players('D')

shuffle(player_stats_list)
# shuffle(player_stats_list)
# shuffle(player_stats_list)
# print(player_stats_list)
# exit()
ga = pyeasyga.GeneticAlgorithm(player_stats_list,population_size=10000)        # initialise the GA with data
...
def fitness(individual, data):
	name,rank,salary,qb,rb,wr,te,dst = '',0,0,0,0,0,0,0
...
for i in range(1, len(players)):
	if players[i] > 0:
		print(player_stats_list[i][0]+"\t"+str(player_stats_list[i][1])+"\t"+str(player_stats_list[i][2]))
		total_sal += player_stats_list[i][2]
		total_rank += player_stats_list[i][1]
print()
print(total_sal)
print(total_rank)
